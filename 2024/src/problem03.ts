export function problem03a(inputfile: string): number {
  ...
  const found: RegExpStringIterator<RegExpExecArray> | null = input.matchAll(regex);
  if (found === null) {
    return 0;
  }
  let sum: number = 0;
...
export function problem03b(inputfile: string): number {
  ...
  const keep = input.split(/do\(\)/).map((block) => {
    return block.substring(0, block.indexOf("don't()") === -1 ? block.length : block.indexOf("don't()"));
  });
  const regex = /mul\((\d+),(\d+)\)/gi;
  let sum: number = 0;
  keep.forEach((block) => {
    const found: RegExpStringIterator<RegExpExecArray> | null = block.matchAll(regex);
    if (found === null) {
      return 0;
    }
    found.forEach((line) => {
