...
function compute(board: string[][], value: number) : number {
  const vals: number[][] = board.map((row) => row.map((elem) => parseInt(elem)))
  const sum = vals.flat().filter((num) => num > 0).reduce((sum, value) => sum + value);
  return sum * value
}
...
export async function four_b() : Promise<number> {
  ...
  for (const input of inputs) {
    ...
    if (boards.length == 1){
      ...
      boards = boards.filter((board) => !(row_check(board) || col_check(board)))
    }
    // console.log(boards);
  }
